...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/sr_wave_64_128_patch.json',
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for i, train_data in enumerate(train_loader):
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for _,  val_data in enumerate(val_loader):
                        ...
                        if isinstance(sr_img, dict):
                            ...
                            for k,v in sr_img.items():
                                ...
                                lpips[k] = Metrics.calculate_lpips(gen_img, hr_img)
                                logger_val_custom.info('<epoch:{:3d}, iter:{:8,d}, type: {}> psnr: {:.4e}, ssim: {:.4e}, lpips: {:.4e}'.format(current_epoch, current_step, str(k), psnr[k], ssim[k],lpips[k]))
                            avg_psnr += psnr['mean']
                            avg_ssim += ssim['mean']
                            ...
                            all_psnr.append(Metrics.calculate_psnr(sr_img, hr_img))
                            all_ssim.append(Metrics.calculate_ssim(sr_img, hr_img))
                            all_lpips.append(Metrics.calculate_ssim(sr_img, hr_img))
                    ...
                    tb_logger.add_scalar("PSNR (dB)", avg_psnr, current_step)
        ...
        for _,  val_data in enumerate(val_loader):
            ...
            all_psnr.append(eval_psnr)
            all_ssim.append(eval_ssim)
            all_lpips.append(eval_lpips)
        ...
        logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.4e}, ssim：{:.4e}, lpips：{:.4e}'.format(
            current_epoch, current_step, avg_psnr, avg_ssim, avg_lpips))
        with open(os.path.join(result_path,'res.txt'), 'w+') as f:
            # write elements of list
            for i in range(len(all_psnr)):
                f.write(f'PSNR: {all_psnr[i]} dB  SSIM: {all_ssim[i]} LPIPS: {all_lpips[i]}\n\n')
        f.close()
